(function (global) {
  ...
  var STR = {
    en: {
      ...
      "method.backend.p": "A small Python server (backend/server.py) reads the same catalog files and answers JSON: /api/stats, /api/majors, /api/search, /api/quiz/draw. There is no login database. Your likes still stay only in this browser.",
    ...
    zh: {
      ...
      "method.backend.p": "一个小的 Python 服务（backend/server.py）读取同一份目录文件，用 JSON 回答：/api/stats、/api/majors、/api/search、/api/quiz/draw。没有登录数据库。你点的兴趣仍只存在这台浏览器里。",
